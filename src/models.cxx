...
NEMLModel_sd::NEMLModel_sd(std::shared_ptr<LinearElasticModel> emodel,
                           std::shared_ptr<Interpolate> alpha,
                           bool truesdell)
  : NEMLModel(), elastic_(emodel), alpha_(alpha), truesdell_(truesdell)
{
}
...
int
NEMLModel_sd::update_ld_inc(const double * const d_np1,
                            const double * const d_n,
                            const double * const w_np1,
                            const double * const w_n,
                            double T_np1,
                            double T_n,
                            double t_np1,
                            double t_n,
...
size_t
NEMLModel_sd::nstore() const
{
  return nhist() + 6;
}

int
NEMLModel_sd::init_store(double * const store) const
{
  init_hist(&store[0]);
...
double
NEMLModel_sd::alpha(double T) const
{
  return alpha_->value(T);
}

const std::shared_ptr<const LinearElasticModel>
NEMLModel_sd::elastic() const
{
  return elastic_;
}

int
NEMLModel_sd::elastic_strains(const double * const s_np1,
                              double T_np1,
...
int
NEMLModel_sd::set_elastic_model(std::shared_ptr<LinearElasticModel> emodel)
{
  elastic_ = emodel;
  return 0;
}
...
int
NEMLModel_sd::calc_tangent_(const double * const D,
...
NEMLModel_ldi::NEMLModel_ldi() : NEMLModel() {}

NEMLModel_ldi::~NEMLModel_ldi() {}

int
NEMLModel_ldi::update_sd(const double * const e_np1,
...
size_t
NEMLModel_ldi::nstore() const
{
  return nhist();
}

int
NEMLModel_ldi::init_store(double * const store) const
{
  init_hist(&store[0]);
  return 0;
}
...
int
SubstepModel_sd::update_sd(const double * const e_np1,
                           const double * const e_n,
                           double T_np1,
                           double T_n,
                           double t_np1,
                           ...
                           double p_n)
{
  ...
  while (cs < tf)
  {
    ...
    for (size_t i = 0; i < 6; i++)
      e_next[i] = e_n[i] + sm * e_diff[i];
    T_next = T_n + sm * T_diff;
    t_next = t_n + sm * t_diff;

    // Try updating
    int ier = update_step(e_next,
                          e_past,
                          T_next,
                          T_past,
                          t_next,
...
int
SubstepModel_sd::update_step(const double * const e_np1,
                             const double * const e_n,
                             double T_np1,
                             double T_n,
                             double t_np1,
                             ...
                             double p_n)
{
  ...
  if (elastic_step(ts, e_np1, e_n, T_np1, T_n, t_np1, t_n, s_n, h_n))
  {
    ...
    // Energy and work
    work_and_energy(
        ts, e_np1, e_n, T_np1, T_n, t_np1, t_n, s_np1, s_n, h_np1, h_n, u_np1, u_n, p_np1, p_n);
    delete ts;
    return 0;
  }
  ...
  ier = update_internal(x, e_np1, e_n, T_np1, T_n, t_np1, t_n, s_np1, s_n, h_np1, h_n);
...
std::string
SmallStrainElasticity::type()
{
  return "SmallStrainElasticity";
}

ParameterSet
SmallStrainElasticity::parameters()
{
  ParameterSet pset(SmallStrainElasticity::type());

  pset.add_parameter<NEMLObject>("elastic");

  pset.add_optional_parameter("alpha", std::make_shared<ConstantInterpolate>(0.0));
...
SmallStrainElasticity::initialize(ParameterSet & params)
{
...
size_t
SmallStrainElasticity::nhist() const
{
  return 0;
}

int
SmallStrainElasticity::init_hist(double * const hist) const
{
  return 0;
}

int
SmallStrainElasticity::update_sd(const double * const e_np1,
                                 const double * const e_n,
                                 double T_np1,
                                 double T_n,
                                 double t_np1,
...
SmallStrainPerfectPlasticity::SmallStrainPerfectPlasticity(
    ...
    bool force_divide,
    bool truesdell)
  : SubstepModel_sd(elastic, alpha, truesdell, tol, miter, verbose, max_divide, force_divide),
    surface_(surface),
    ys_(ys)
{
}
...
std::string
SmallStrainPerfectPlasticity::type()
{
  return "SmallStrainPerfectPlasticity";
}

ParameterSet
SmallStrainPerfectPlasticity::parameters()
{
...
std::unique_ptr<NEMLObject>
SmallStrainPerfectPlasticity::initialize(ParameterSet & params)
{
...
size_t
SmallStrainPerfectPlasticity::nhist() const
{
  return 0;
}

int
SmallStrainPerfectPlasticity::init_hist(double * const hist) const
{
  return 0;
}

TrialState *
SmallStrainPerfectPlasticity::setup(const double * const e_np1,
                                    const double * const e_n,
                                    double T_np1,
                                    double T_n,
                                    double t_np1,
                                    ...
                                    const double * const h_n)
{
  ...
  make_trial_state(e_np1, e_n, T_np1, T_n, t_np1, T_n, s_n, h_n, *tss);
  return tss;
}

bool
SmallStrainPerfectPlasticity::elastic_step(const TrialState * ts,
...
int
SmallStrainPerfectPlasticity::update_internal(const double * const x,
                                              const double * const e_np1,
                                              const double * const e_n,
                                              double T_np1,
                                              double T_n,
...
int
SmallStrainPerfectPlasticity::strain_partial(const TrialState * ts,
                                             const double * const e_np1,
                                             const double * const e_n,
                                             double T_np1,
                                             double T_n,
...
int
SmallStrainPerfectPlasticity::work_and_energy(const TrialState * ts,
                                              const double * const e_np1,
                                              const double * const e_n,
                                              double T_np1,
                                              double T_n,
                                              ...
                                              double p_n)
{
  ...
  double dp[6];
  for (size_t i = 0; i < 6; i++)
    dp[i] = e_np1[i] - ee_np1[i] - tss->ep_n[i];
...
size_t
SmallStrainPerfectPlasticity::nparams() const
{
  return 7;
}

int
SmallStrainPerfectPlasticity::init_x(double * const x, TrialState * ts)
{
...
int
SmallStrainPerfectPlasticity::RJ(const double * const x,
                                 TrialState * ts,
                                 double * const R,
                                 double * const J)
{
  ...
  double T[6];
  for (int i = 0; i < 6; i++)
    T[i] = tss->e_np1[i] - tss->ep_n[i] - df[i] * dg;
...
// Getter
double
SmallStrainPerfectPlasticity::ys(double T) const
{
  return ys_->value(T);
}
...
int
SmallStrainPerfectPlasticity::make_trial_state(const double * const e_np1,
                                               const double * const e_n,
                                               double T_np1,
                                               double T_n,
                                               double t_np1,
                                               ...
                                               SSPPTrialState & ts)
{
  ...
  for (size_t i = 0; i < 6; i++)
    ts.ep_n[i] = e_n[i] - ts.ep_n[i];

  double de[6];
  sub_vec(e_np1, e_n, 6, de);
  mat_vec(ts.C, 6, de, 6, ts.s_tr);
  for (size_t i = 0; i < 6; i++)
    ts.s_tr[i] = s_n[i] + ts.s_tr[i];
...
std::string
SmallStrainRateIndependentPlasticity::type()
{
  return "SmallStrainRateIndependentPlasticity";
}

ParameterSet
SmallStrainRateIndependentPlasticity::parameters()
{
...
std::unique_ptr<NEMLObject>
SmallStrainRateIndependentPlasticity::initialize(ParameterSet & params)
{
...
size_t
SmallStrainRateIndependentPlasticity::nhist() const
{
  return flow_->nhist();
}

int
SmallStrainRateIndependentPlasticity::init_hist(double * const hist) const
{
  return flow_->init_hist(hist);
}

TrialState *
SmallStrainRateIndependentPlasticity::setup(const double * const e_np1,
                                            const double * const e_n,
                                            double T_np1,
                                            double T_n,
                                            double t_np1,
...
bool
SmallStrainRateIndependentPlasticity::elastic_step(const TrialState * ts,
                                                   const double * const e_np1,
                                                   const double * const e_n,
                                                   double T_np1,
                                                   double T_n,
...
int
SmallStrainRateIndependentPlasticity::update_internal(const double * const x,
                                                      const double * const e_np1,
                                                      const double * const e_n,
                                                      double T_np1,
                                                      double T_n,
...
int
SmallStrainRateIndependentPlasticity::strain_partial(const TrialState * ts,
                                                     const double * const e_np1,
                                                     const double * const e_n,
                                                     double T_np1,
                                                     double T_n,
...
int
SmallStrainRateIndependentPlasticity::work_and_energy(const TrialState * ts,
                                                      const double * const e_np1,
                                                      const double * const e_n,
                                                      double T_np1,
                                                      double T_n,
                                                      ...
                                                      double p_n)
{
  ...
  double dp[6];
  for (size_t i = 0; i < 6; i++)
    dp[i] = e_np1[i] - ee_np1[i] - tss->ep_tr[i];
...
size_t
SmallStrainRateIndependentPlasticity::nparams() const
{
...
int
SmallStrainRateIndependentPlasticity::init_x(double * const x, TrialState * ts)
{
...
int
SmallStrainRateIndependentPlasticity::RJ(const double * const x,
                                         TrialState * ts,
                                         double * const R,
                                         double * const J)
{
...
const std::shared_ptr<const LinearElasticModel>
SmallStrainRateIndependentPlasticity::elastic() const
{
  return elastic_;
}

int
SmallStrainRateIndependentPlasticity::make_trial_state(const double * const e_np1,
                                                       const double * const e_n,
                                                       double T_np1,
                                                       double T_n,
                                                       double t_np1,
...
// Implementation of small strain rate independent plasticity
//
SmallStrainCreepPlasticity::SmallStrainCreepPlasticity(std::shared_ptr<LinearElasticModel> elastic,
...
std::string
SmallStrainCreepPlasticity::type()
{
  return "SmallStrainCreepPlasticity";
}

ParameterSet
SmallStrainCreepPlasticity::parameters()
{
...
std::unique_ptr<NEMLObject>
SmallStrainCreepPlasticity::initialize(ParameterSet & params)
{
...
size_t
SmallStrainCreepPlasticity::nhist() const
{
...
int
SmallStrainCreepPlasticity::init_hist(double * const hist) const
{
...
int
SmallStrainCreepPlasticity::update_sd(const double * const e_np1,
                                      const double * const e_n,
                                      double T_np1,
                                      double T_n,
                                      double t_np1,
                                      ...
                                      double p_n)
{
  ...
  ier = creep_->update(s_np1, creep_new, creep_old, T_np1, T_n, t_np1, t_n, B);
  if (ier != 0)
    return ier;
...
size_t
SmallStrainCreepPlasticity::nparams() const
{
...
int
SmallStrainCreepPlasticity::init_x(double * const x, TrialState * ts)
{
...
int
SmallStrainCreepPlasticity::RJ(const double * const x,
                               TrialState * ts,
                               double * const R,
                               double * const J)
{
  ...
  ier = creep_->update(s_np1, creep_new, creep_old, tss->T_np1, tss->T_n, tss->t_np1, tss->t_n, B);
  if (ier != 0)
    return ier;
...
int
SmallStrainCreepPlasticity::make_trial_state(const double * const e_np1,
                                             const double * const e_n,
                                             double T_np1,
                                             double T_n,
                                             double t_np1,
...
int
SmallStrainCreepPlasticity::form_tangent_(double * const A, double * const B, double * const A_np1)
{
...
int
SmallStrainCreepPlasticity::set_elastic_model(std::shared_ptr<LinearElasticModel> emodel)
{
  elastic_ = emodel;
...
std::string
GeneralIntegrator::type()
{
  return "GeneralIntegrator";
}

ParameterSet
GeneralIntegrator::parameters()
{
...
std::unique_ptr<NEMLObject>
GeneralIntegrator::initialize(ParameterSet & params)
{
...
TrialState *
GeneralIntegrator::setup(const double * const e_np1,
                         const double * const e_n,
                         double T_np1,
                         double T_n,
                         double t_np1,
...
bool
GeneralIntegrator::elastic_step(const TrialState * ts,
                                const double * const e_np1,
                                const double * const e_n,
                                double T_np1,
                                double T_n,
...
int
GeneralIntegrator::update_internal(const double * const x,
                                   const double * const e_np1,
                                   const double * const e_n,
                                   double T_np1,
                                   double T_n,
...
int
GeneralIntegrator::strain_partial(const TrialState * ts,
                                  const double * const e_np1,
                                  const double * const e_n,
                                  double T_np1,
                                  double T_n,
...
int
GeneralIntegrator::work_and_energy(const TrialState * ts,
                                   const double * const e_np1,
                                   const double * const e_n,
                                   double T_np1,
                                   double T_n,
...
size_t
GeneralIntegrator::nhist() const
{
  return rule_->nhist();
}

int
GeneralIntegrator::init_hist(double * const hist) const
{
  return rule_->init_hist(hist);
}

size_t
GeneralIntegrator::nparams() const
{
  return 6 + nhist();
}

int
GeneralIntegrator::init_x(double * const x, TrialState * ts)
{
...
int
GeneralIntegrator::RJ(const double * const x, TrialState * ts, double * const R, double * const J)
{
  GITrialState * tss = static_cast<GITrialState *>(ts);
  ...
  // More vectorization
  double dt = tss->dt;
  for (int i = 0; i < nhist * nhist; i++)
    J22[i] *= dt;
  for (int i = 0; i < nhist; i++)
    J22[CINDEX(i, i, nhist)] -= 1.0;
...
int
GeneralIntegrator::make_trial_state(const double * const e_np1,
                                    const double * const e_n,
                                    double T_np1,
                                    double T_n,
                                    double t_np1,
...
int
GeneralIntegrator::set_elastic_model(std::shared_ptr<LinearElasticModel> emodel)
{
  elastic_ = emodel;
...
std::unique_ptr<NEMLObject>
KMRegimeModel::initialize(ParameterSet & params)
{
...
int
KMRegimeModel::update_sd(const double * const e_np1,
                         const double * const e_n,
                         double T_np1,
                         double T_n,
                         double t_np1,
                         ...
                         double p_n)
{
  ...
  for (size_t i = 0; i < gs_.size(); i++)
  {
    if (g < gs_[i])
    {
      return models_[i]->update_sd(e_np1,
                                   e_n,
                                   T_np1,
                                   T_n,
                                   t_np1,
  ...
  return models_.back()->update_sd(
      e_np1, e_n, T_np1, T_n, t_np1, t_n, s_np1, s_n, h_np1, h_n, A_np1, u_np1, u_n, p_np1, p_n);
}

size_t
KMRegimeModel::nhist() const
{
  return models_[0]->nhist();
}

int
KMRegimeModel::init_hist(double * const hist) const
{
  return models_[0]->init_hist(hist);
}

double
KMRegimeModel::activation_energy_(
    const double * const e_np1, const double * const e_n, double T_np1, double t_np1, double t_n)
{
  double dt = t_np1 - t_n;
...
int
KMRegimeModel::set_elastic_model(std::shared_ptr<LinearElasticModel> emodel)
{
  elastic_ = emodel;
  int ier;
