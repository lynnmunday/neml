...
std::shared_ptr<NEMLModel>
parse_string(std::string input)
{
...
std::unique_ptr<NEMLModel>
parse_string_unique(std::string input, std::string mname)
{
  // Parse the string to the rapidxml representation
  rapidxml::xml_document<> doc;
  doc.parse<0>(&input[0]);
...
std::shared_ptr<NEMLModel>
parse_xml(std::string fname, std::string mname)
{
  // Parse the XML file
...
std::unique_ptr<NEMLModel>
parse_xml_unique(std::string fname, std::string mname)
{
  // Parse the XML file
  rapidxml::file<> xmlFile(fname.c_str());
  rapidxml::xml_document<> doc;
...
get_object_unique(const rapidxml::xml_node<> * node)
{
...
get_object(const rapidxml::xml_node<> * node)
{
  // Special case: could be a ConstantInterpolate
  std::string type = get_type_of_node(node);
  if (type == "none")
...
ParameterSet
get_parameters(const rapidxml::xml_node<> * node)
{
  std::string type = get_type_of_node(node);
  ...
  for (rapidxml::xml_node<> * child = node->first_node(); child; child = child->next_sibling())
  {
    ...
    switch (pset.get_object_type(name))
    {
      ...
      case TYPE_DOUBLE:
        pset.assign_parameter(name, get_double(child)); // get_value, dynamic cast to type
                                                        // requested.
        break;
      case TYPE_INT:
...
std::vector<std::shared_ptr<NEMLObject>>
get_vector_object(const rapidxml::xml_node<> * node)
{
...
double
get_double(const rapidxml::xml_node<> * node)
{
  try
  {
...
int
get_int(const rapidxml::xml_node<> * node)
{
  try
  {
    std::string text = get_string(node);
...
std::vector<double>
get_vector_double(const rapidxml::xml_node<> * node)
{
  try
  {
    std::string text = get_string(node);
...
bool
get_bool(const rapidxml::xml_node<> * node)
{
  std::string text = get_string(node);
...
std::string
get_string(const rapidxml::xml_node<> * node)
{
...
list_systems
get_slip(const rapidxml::xml_node<> * node)
{
  list_systems groups;
...
std::string
get_type_of_node(const rapidxml::xml_node<> * node)
{
  for (auto attributes = node->first_attribute(); attributes;
...
std::vector<int>
split_string_int(std::string sval)
{
  std::vector<std::string> splits;
  std::stringstream ss(sval);
  std::string temp;
